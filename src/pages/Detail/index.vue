        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 点击加入购物车, 路由跳转前  发送请求  带着产品的信息id  类别-->
...
export default {
  ...
  methods: {
    changActive(saleAttrValue, arr) {
      // 遍历排他
      // 遍历全部售卖属性值 isChecked 设置为0
      arr.forEach((item) => {
        item.isChecked = 0;
      });
      // 点击的售卖属性 isChecked 设置为1
      saleAttrValue.isChecked = 1;
    },
    changeSkuNum(event) {
      //表单元素修改产品个数
      //判断用户输入   字符串 or 数字   对格式限制 整数 event.target.value
      let value = event.target.value * 1;
      //  如果用户输入非法的含字母 出现了NaN    not a  number
      if (isNaN(value) || value < 1) {
        console.log("您的输入格式有误");
        // 如果不是非法的
        this.skuNum = 1;
      } else {
        // 大于一   但是需要判断  如果是小数 应该parseInt
